...
仕訳帳に、以下の9件の仕訳が正しく表示されることを確認しました:

| 取引日 | 借方勘定科目 | 借方金額 | 貸方勘定科目 | 貸方金額 | 摘要 | 元データ |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 2025-04-15 | 現金 | 100,000 | 売上高 | 100,000 | テスト仕訳 | 振替伝票 |
| 2025-05-20 | 普通預金 | 50,000 | 売上高 | 50,000 | テスト2件目 | 振替伝票 |
| 2025-07-28 | 会議費 | 16,665 | 普通預金 | 16,665 | セブンイレブン | その他 |
| 2025-08-01 | 普通預金 | 93,600 | 売上高 | 93,600 | パワー振込 | その他 |
| 2025-08-01 | 普通預金 | 220 | 売上高 | 220 | 振込入金 | その他 |
| 2025-11-11 | 通信費 | 5,500 | 普通預金 | 5,500 | サーバー利用料 | その他 |
| 2025-11-12 | 普通預金 | 100,000 | 売上高 | 100,000 | コンサルティング報酬 | その他 |
| 2025-11-13 | 広告費 | 50,000 | 普通預金 | 50,000 | - | その他 |
| 2025-11-14 | 普通預金 | 80,000 | 売上高 | 80,000 | - | その他 |

**✅ すべての処理済み取引が仕訳帳に反映されています**
**✅ 勘定科目が正しく表示されています**

### 振替伝票一覧の表示確認 ✅ 成功

振替伝票一覧には、振替伝票から直接入力した仕訳のみが表示されることを確認しました:

| 取引日 | 摘要 | 借方勘定科目 | 借方金額 | 貸方勘定科目 | 貸方金額 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 2025-08-01 | 振込入金 | 普通預金 | 220 | 売上高 | 220 |
| 2025-05-20 | テスト2件目 | 普通預金 | 50,000 | 売上高 | 50,000 |
| 2025-04-15 | テスト仕訳 | 現金 | 100,000 | 売上高 | 100,000 |

**✅ 取引明細から登録した仕訳が除外されています**
...
5. `migrate_to_general_ledger.py`（新規作成）
   - 既存の`journal_entries`テーブルのデータを`general_ledger`テーブルに移行するスクリプト

6. `FIXES_SUMMARY.md`（新規作成）
   - 修正内容の詳細をまとめたドキュメント
...
**重要**: 本番環境にデプロイする際は、必ず`migrate_to_general_ledger.py`を実行して、既存の処理済み取引を`general_ledger`テーブルに移行してください。
...
報告いただいた3つの問題について、すべて修正が完了しました。

1.  ✅ 仕訳帳に勘定科目が正しく表示されるようになりました
2.  ✅ 振替伝票一覧から取引明細の仕訳が除外されました
3.  ✅ 既存の処理済み取引が仕訳帳に反映されるようになりました

今後、新しく取引明細を登録する際は、自動的に`general_ledger`テーブルに保存されるため、仕訳帳に正しく表示されます。
